...
![effect-original.avif](/img/opencv/effect-original.avif)
...
创建Mat实例时，可以传入`size, type`或者`rows, cols, type`，一般用默认构造方式：
...
> * data操作只对连续的Mat有效，使用前应该用`isContinuous`函数检查。
